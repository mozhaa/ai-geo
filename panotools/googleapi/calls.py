...
async def single_image_search(
    lat: float, lng: float, radius: float = 100, session: Optional[aiohttp.ClientSession] = None
) -> Any:
...
async def get_metadata(panoid: str, session: Optional[aiohttp.ClientSession] = None) -> Any:
...
async def get_tile(
    panoid: str, x: int, y: int, zoom: int, session: Optional[aiohttp.ClientSession] = None
) -> Image.Image:
    url = (
